...
class VADFilter:
    ...
    def trim_silence(self, audio: np.ndarray) -> tuple[np.ndarray, float]:
        """
        Takes raw audio array, trims leading and trailing silence.
        Returns: (trimmed_audio, silence_ratio)
        """
        if len(audio) == 0:
            return audio, 1.0
            
        # Silero expects float32 tensor in [-1, 1]
        ...
        # Get overall start and end (keeping mid-sentence pauses)
